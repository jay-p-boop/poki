...
export default {
  ...
  if (this.micButtonDisabled) {
    return;  // Verhindert, dass die Methode ausgeführt wird, wenn der Button deaktiviert ist
  }

  this.micButtonDisabled = true;  // Deaktiviert den Button
  ...
  // Setzt einen Timeout, um die Spracherkennung nach 4 Sekunden zu stoppen
  const recognitionTimeout = setTimeout(() => {
    recognition.stop();
    this.micButtonDisabled = false; 
    this.isListening = false;// Aktiviert den Button wieder
  }, 4000);

  recognition.onresult = (event) => {
    this.isListening = false;
    this.micButtonDisabled = false; 
    clearTimeout(recognitionTimeout);  // Löscht den Timeout, wenn ein Ergebnis erzielt wird
    ...
    const closestPokemon = this.findClosestPhoneticPokemon(spokenWord);  // Verwendet die neue Methode
    ...
    if (this.learningMode) {
      ...
      console.log(closestPokemon.name)
  ...
  recognition.onerror = (event) => {
    ...
    clearTimeout(recognitionTimeout);  // Löscht den Timeout im Fehlerfall
    this.micButtonDisabled = false;  // Aktiviert den Button wieder im Fehlerfall
  };
}
